# -*- coding: utf-8 -*-
"""
Created on Thu Sep  7 19:20:24 2023
...
def find_title_104(key_txt,area=''):
    area_dict = {'台北市': 6001001000,'新北市': 6001002000,'桃園市': 6001005000,'台中市': 6001008000,'彰化縣': 6001010000,'台南市': 6001014000,'高雄市': 6001016000}
    if area !='':
        if len(area.split(' '))==1:
            areacode=area_dict[area]
        else:
            area_split=area.split(' ')
            areaconvert=[area_dict[i] for i in area_split]
            areacode=''
            for j in range(len(areaconvert)):
                if j==0:
                    areacode=areacode+str(areaconvert[j])
                else:
                    areacode=areacode+'%2C'+str(areaconvert[j])
    else:
        areacode=area
    today = get_todate()
    path_csv = "%s" % os.getcwd()+'/jobs_csv/'+today+'-'+key_txt+' '+area+'-104HR'
    ...
    find_page_url = 'https://www.104.com.tw/jobs/search/?ro=0&kwop=7&keyword={0}&expansionType=area%2Cspec%2Ccom%2Cjob%2Cwf%2Cwktm&area={1}&order=15&asc=0&page=1&mode=s&jobsource=2018indexpoc&searchTempExclude=2&langFlag=0&langStatus=0&recommendJob=1&hotJob=1 '.format(key,areacode)
    ...
    for i in tqdm(range(1, get_sum_page+1)):  #set page 1 to find all max page ,tqdm讀取進度條
        url = 'https://www.104.com.tw/jobs/search/?ro=0&kwop=7&keyword={0}&expansionType=area%2Cspec%2Ccom%2Cjob%2Cwf%2Cwktm&area={1}&order=15&asc=0&page={2}&mode=s&jobsource=2018indexpoc&searchTempExclude=2&langFlag=0&langStatus=0&recommendJob=1&hotJob=1 '.format(key,areacode, i) 
        ...
        for title_1 in soup.select('.b-block__left'):
            ...
            if title_1.select('.b-list-inline.b-clearfix.job-list-item__company') != soup.select('.b-block__left')[0].select('.b-list-inline.b-clearfix.job-list-item__company'):
                ...
                with open(path_csv + '.csv', mode='a+', newline='', encoding='utf-8-sig') as employee_file: #w
                    employee_writer = csv.writer(employee_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
while True:
    ...
    print(f.renderText(' 104 HR '))
    print('歡迎使用104爬蟲程式')
    keyword=input('請輸入要搜尋的關鍵字，取消請輸入0：')
    if keyword=='0':
        print('已經取消工作')
        break
    andserch=True
    andserchinput=input('請輸入是否需要多從搜尋("Y" or "N")，取消請輸入0：')
    if andserchinput=='0':
        print('已經取消工作')
        break
    elif andserchinput=='Y':
        area=input('請輸入要搜尋的地區，取消請輸入0：')
        if area=='0':
            print('已經取消工作')
            break
        else:
            print('您的搜尋條件:'+keyword+' & '+area)
            find_title_104(keyword,area)
            break
    elif andserchinput=='N':
        print('您的搜尋條件:'+keyword)
        find_title_104(keyword)
        break
